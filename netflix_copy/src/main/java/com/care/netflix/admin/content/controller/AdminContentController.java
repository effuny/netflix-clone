package com.care.netflix.admin.content.controller;
...
public class AdminContentController {
	...
	public String content_Insertgo(Model model,@RequestParam("category_name") String category_name,HttpServletRequest request) throws Exception{
		...
		/*
		 * model.addAttribute("actorList",adminActorService.getAllActor(map)); //배우 목록
		 * 전달
		 * model.addAttribute("directorList",adminDirectorService.getAllDirector(map));
		 * //감독 목록 전달
		 * model.addAttribute("genreList",adminGenreService.getAllGenre(map)); //장르 목록
		 * 전달
		 */		
		request.setAttribute("category_name", category_name);
